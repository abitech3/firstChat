...
from .models import Question, Choice, Article, Page_Setting
...
def app_setting(request):
     if request.method == "POST":
         form = App_Customize(request.POST , request.FILES)
         if form.is_valid():
             form.save()
             form = App_Customize()           
...
def chatbot(request):
    page = Page_Setting.objects.filter(title= 'hammed')
    return render(request , 'bot.html', { 'pages' : page}, )

def lead(request):
    page = Page_Setting.objects.filter(title= 'hammed')
    return render(request , 'lead.html', {'pages' : page} ,)

def setting(request):
    page = Page_Setting.objects.filter(title= 'hammed')
    return render(request , 'setting.html' , {'pages' : page}, )

def header_nav(request):
    page = Page_Setting.objects.filter(title= 'hammed')
    return render(request , 'header.html',{'pages' : page} )
...
def botsetting(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            #  return redirect('{}?submitted=True'.format(reverse('signup:login')))
             msg = "you have been successfuly update your App"
             return render(request, 'botset.html', {'msg': msg , 'form' : form , 'pages' : page})
        ...
        form = Bot_Set()           
    # Ensure that there's a default response (e.g., rendering a template)
    return render(request, 'botset.html', {'form': form , 'pages' : page})
...
def dashboard(request):
    page = Page_Setting.objects.filter(title='hammed')
    print(page)
    return render(request, 'dashboard.html', {'pages': page })
